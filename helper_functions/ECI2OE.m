function oe=ECI2OE(r_ijk, v_ijk, mu)
    %{
    ...
    Inputs:
        r_ijk - 3x1 vector of radius in ECI frame [km] v_ijk - 3x1 vector of velocity in ECI frame [km]
        mu - gravitational parameter of central body [km^-3/s^2]
    
    Outputs:
        oe = struct with the following fields:
        a - semi-major axis of orbit [km] e - eccentricity of orbit
        i - inclination of orbit [deg]
        Om - right ascension of the ascending node [deg] w - argument of periapsis [deg]
        w - argument of periapsis [deg]
        anom - true anomaly [deg]
        ang - extra placeholder angle for special cases [deg]
    
    
    This function is able to handle any orbit type, including equatorial and circular orbits. The output ang will output the following values for each orbit type:
    ...
    i = acosd(hVec(3)/h);
    Om = acosd(nVec(1)/n);
    w = acosd(dot(nVec, eVec)/(n*e)); % NUMERICAL CLAMPING REQUIRED?????
